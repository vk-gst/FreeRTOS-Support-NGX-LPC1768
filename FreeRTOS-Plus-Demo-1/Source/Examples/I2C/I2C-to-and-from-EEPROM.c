...
/* EEPROM parameters. */
#define i2cTOTAL_EEPROM_SIZE 	( 4*1024UL )
...
uint8_t* vI2C_EEPROMTest( Peripheral_Descriptor_t xI2CPort,uint8_t *message )
{
...
int32_t lReturned;
	...
	for( ulPage = 0UL; ulPage < ( i2cTOTAL_EEPROM_SIZE / i2cPAGE_SIZE ); ulPage++ )
	{
		for( ucByte = 0U; ucByte < i2cPAGE_SIZE; ucByte++ )
		{
			...
			ucDataBuffer[ ucByte + 2 ] = message[ ucValue ];
			ucValue++;
		}
	...
	//{
		...
		/* Read a page back from the EEPROM. */
		lReturned = FreeRTOS_read( xI2CPort, ucDataBuffer, strlen(message) );
		configASSERT( lReturned == strlen(message) );

		FreeRTOS_ioctl( xI2CPort, ioctlRELEASE_WRITE_MUTEX, ( void * ) NULL);
		return ucDataBuffer;
...
const uint32_t ulMaxDelay = ( 500UL / portTICK_RATE_MS );
	...
	ucDataBuffer[ 1 ] = prvSetSlaveAddress( xI2CPort, ulByteAddress );
	ucDataBuffer[0]=*MSB;
