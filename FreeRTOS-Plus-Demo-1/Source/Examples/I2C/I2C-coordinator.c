...
void vI2CTaskStart( void *xqueue )
{
	...
	xTaskCreate( 	prvI2CTask,								/* The task that uses the I2C peripheral. */
					...
					&xHandle );									/* A handle to the task being created is not required, so just pass in NULL. */
}
/*-----------------------------------------------------------*/
...
static void prvI2CTask( void *pvParameters )
{
	...
	const uint32_t ulMaxDelay = 500UL / portTICK_RATE_MS;
	 ...
	 xQueueHandle txqueue = (xQueueHandle)pvParameters;

	const uint8_t *message = "linking uart and IC";
	...
	eeprom_data=vI2C_EEPROMTest( xI2CPort,message );
	configASSERT( eeprom_data );

	xQueueSend( txqueue, &eeprom_data, 0 );
