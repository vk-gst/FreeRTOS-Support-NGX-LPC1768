...
THIS CODE WAS REMOVED BY _RB_ AS THE BUFFERS ARE MANUALLY PLACED IN THE AHB RAM
BY THE APPLICATION CODE.

/* EMAC local DMA buffers */
/** Rx buffer data */
static uint32_t rx_buf[EMAC_NUM_RX_FRAG][EMAC_ETH_MAX_FLEN>>2] = { { 0 }, { 0 } };
/** Tx buffer data */
static uint32_t tx_buf[EMAC_NUM_TX_FRAG][EMAC_ETH_MAX_FLEN>>2] = { { 0 }, { 0 } };

uint32_t rx_buf[EMAC_NUM_RX_FRAG][EMAC_ETH_MAX_FLEN>>2];
uint32_t tx_buf[EMAC_NUM_TX_FRAG][EMAC_ETH_MAX_FLEN>>2];
#endif /* 0 */
...
int32_t EMAC_SetPHYMode(uint32_t ulPHYMode)
{
	...
	if (((id1 << 16) | (id2 & 0xFFF0)) == 0x0007C0F0UL /*boardPHY_ID*/) {
		switch(ulPHYMode){
		case EMAC_MODE_AUTO:
